...
func (c VpnCredential) caPayload(ca []byte, uuid string) payload {
	return payload{
		...
		DisplayName:         "Trust Networks VPN CA",
...
func (c VpnCredential) GetMCIpsec(client *Client) ([]CredentialPayload, error) {
	...
	pays := []payload{
		...
		payload{
			...
			UserDefinedName: "Trust Networks (UK) IPsec",
			VPNType:         "IKEv2",
		},
		...
		payload{
			...
			Version:         1,
			Proxies:         c.proxyConfig(),
			UserDefinedName: "Trust Networks (US) IPsec",
			VPNType:         "IKEv2",
		},
	}
	...
	p := configuration{
		Payloads:          pays,
		DisplayName:       "Trust Networks VPN",
		Identifier:        device,
...
func (c VpnCredential) GetMCOpenVpn(client *Client) ([]CredentialPayload, error) {
	...
	pays := []payload{
		...
		payload{
			...
			UUID:            ukVpnPayUuid,
			Version:         1,
			UserDefinedName: "Trust Networks (UK) OpenVPN",
			VPNType:         "VPN",
		...
		payload{
			...
			Version:         1,
			Proxies:         c.proxyConfig(),
			UserDefinedName: "Trust Networks (US) OpenVPN",
			VPNType:         "VPN",
			VPNSubType:      "net.openvpn.connect.app",
			VendorConfig: c.openVPNVendorConfig(
				blockPem(creds.Ca),
